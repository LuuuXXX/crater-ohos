...
Crater-OHOS 采用 **服务端独立 + Bot 外接** 架构：
...
| Bot 集成 | 内置 GitHub bot | Bot 为独立组件，通过 API 调用 |
| Callback | 无 | 支持 webhook 回调通知 Bot |
| 部署方式 | 一体化部署 | 服务端与 Bot 可独立部署 |
...
crater-ohos 作为独立的服务端，Bot 是外部独立组件，通过 REST API 与服务端进行交互。

#### Bot 调用 API 流程

Bot 通过 HTTP REST API 与服务端交互，典型流程如下：

1. **Bot 接收用户指令**：Bot 监听代码托管平台（如 Gitee、GitHub）的 Issue 评论
2. **Bot 调用 API**：Bot 解析指令后，调用 crater-ohos 的 REST API 创建实验
3. **服务端处理请求**：crater-ohos 验证 Token、创建实验、启动执行
4. **Webhook 回调通知**：实验状态变化时，服务端通过 webhook 主动通知 Bot
5. **Bot 发布结果**：Bot 接收回调后，在 Issue 中发布实验结果

#### Webhook Callback 机制

服务端支持通过 webhook 主动通知 Bot 实验状态变化。在创建实验时，可以指定 callback URL：

```bash
# 创建实验时指定 callback URL
curl -X POST http://localhost:3000/api/v1/experiments \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
...
当实验状态变化时，服务端会向 callback URL 发送 POST 请求：
...
Bot 需要实现 webhook 端点来接收这些通知。

#### 典型交互示例

以 Gitee Bot 为例，完整的交互流程：

```
1. 用户在 Gitee Issue 中评论：@crater-bot run stable beta
   ↓
2. Gitee Bot 接收 webhook 通知，解析指令
   ↓
3. Bot 调用 crater-ohos API 创建实验：
   POST /api/v1/experiments
   {
     "name": "gitee-issue-123",
     "toolchains": ["stable", "beta"],
     "callback_url": "https://gitee-bot.com/webhook"
   }
   ↓
4. crater-ohos 创建实验，返回实验详情
   ↓
5. Bot 在 Issue 中发布评论："实验已创建，等待执行..."
   ↓
6. crater-ohos 执行实验
   ↓
7. 实验完成后，crater-ohos 发送 webhook 到 Bot：
   POST https://gitee-bot.com/webhook
   {
   ...
   ↓
8. Bot 接收回调，在 Issue 中发布实验结果和报告链接
```
...
1. **实现平台监听**：监听代码托管平台的 Issue 评论或其他事件
2. **解析用户指令**：从评论中提取实验参数（工具链、crate 选择等）
3. **调用 crater-ohos API**：使用 Bearer Token 认证，调用 API 创建和管理实验
4. **实现 Webhook 端点**：接收 crater-ohos 的状态通知
5. **发布结果到平台**：将实验结果和报告链接发布到 Issue 或 PR

详细的 Bot 开发指南和参考实现，请参见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)。
...
### 📋 Phase 7: External Bot Support（外部 Bot 支持）- 计划中

Bot 作为独立组件，通过 REST API 与服务端交互：

- [ ] 提供 Bot 开发指南和 API 使用示例
- [ ] Gitee Bot 参考实现（独立仓库）
- [ ] GitHub Bot 参考实现（独立仓库）
- [ ] GitLab Bot 参考实现（独立仓库）
- [ ] Bot SDK（可选，简化 Bot 开发）
...
- 设计文档：[ARCHITECTURE.md](docs/ARCHITECTURE.md)
- API 使用指南：[API_GUIDE.md](docs/API_GUIDE.md)
